...
def plot3d(chm: np.ndarray, parcel_image: typing.IO, adress: str) -> None:
    fig = make_subplots(
        rows=1, cols=2, specs=[[{"type": "image"}, {"type": "surface"}]]
    )

    fig.add_trace(go.Surface(z=chm), row=1, col=2)
    fig.update_layout(
        title=f"3D rendering of {adress}",
        title_x=0.5,
        scene=dict(
            xaxis_title="x (m)",
            yaxis_title="y (m)",
...
def adress_from_input():
    ...
    street = input(
        "Please enter the street of the building " "(ex.: Korenmarkt): "
    )
