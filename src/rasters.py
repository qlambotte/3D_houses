...
def crop_rasters(dsm, dtm, shapes, name):
    ...
    with rasterio.open(f"./data/{name}_dsm_croped.tiff", "w", **out_meta) as dest:
    ...
    out_meta.update({"driver": "GTiff",
                 ...
                 "width": out_image.shape[2],
                 "transform": out_transform})
    with rasterio.open(f"./data/{name}_dtm_croped.tiff", "w", **out_meta) as dest:
        dest.write(out_image)

def do_merge(list_files):
    list_opened_files = []
...
def merge_rasters(name):
    for d in ["dsm", "dtm"]:
        list_files = glob.glob(f"./data/*_{d}_croped.tiff")
        mosaic, out_trans, out_meta = do_merge(list_files)
        with rasterio.open(f"./data/{name}_{d}_croped.tiff", "w", **out_meta) as file:
            file.write(mosaic)

def chm(dtm, dsm, name):
